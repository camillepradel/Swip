...
public class Term{
	...
	/**
	 * 
	 * @return The array with non-nromalized tokens and without the special tokens.
	 */
	...
	private String[] normalizedTokensPropertyWithoutIgnore(String[] usedTokens) {
		...
		for(int i=0; i<usedTokens.length; i++) {			
			try {
				...
				System.out.println("length: " + usedTokens.length);
				for(int k=0; k<usedTokens.length; k++) {
					System.out.println("k-tes: "+ k + "  " + usedTokens[k]);
				}
			}			
			
			normalizedTokensWithoutSpecialTokensBefore[i]= usedTokens[i].toLowerCase();	
		}		
		//check if a word out of the files were in the name and remove it
	 ...
	 * if the method should return the array of nin-normalized tokens without special tokens.
	 ...
	 * This method delets such words and built a new vector which contains the term without them.
	 * 
	 * @return The normilazed token without special words like of, and, or. 
	 */	
